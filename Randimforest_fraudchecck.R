...
train <- FC[ind==1,]
View(train)
train$Undergrad <- as.factor(train$Undergrad)
train$Marital.Status <- as.factor(train$Marital.Status)
train$Urban <- as.factor(train$Urban)
train$Risky_Good <- as.factor(train$Risky_Good)
test  <- FC[ind==2,]
View(test)
test$Undergrad <- as.factor(test$Undergrad)
test$Marital.Status <- as.factor(test$Marital.Status)
test$Urban <- as.factor(test$Urban)
test$Risky_Good <- as.factor(test$Risky_Good)
set.seed(213)
...
confusionMatrix(pred2, test$Risky_Good) # 100 % accuracy on test data # Error Rate in Random Forest Model :
plot(rf)
